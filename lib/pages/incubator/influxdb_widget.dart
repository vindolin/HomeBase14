...
class _InfluxdbWidgetState extends ConsumerState<InfluxdbWidget> {
  ...
  Future<List<List<TimePoint>>> fetchResult() async {
    ...
    for (final fieldName in ['target_temp', 'temp']) {
    ...
    if (false) {
      var response2 = await dio.post(
        ...
        SELECT *
        FROM incubator
        WHERE sensor == 'temp' or sensor == 'target_temp'
        AND (time >= now() - 12h)
      ''',
          },
        ),
      ...
      final fieldNames = ['target_temp', 'temp'];
        ...
        if (snapshot.hasData) {
          ...
          return Center(
            child: SfCartesianChart(
              ...
              primaryYAxis:
                  NumericAxis(numberFormat: NumberFormat('#0'), labelFormat: '{value}°C', minimum: 20, maximum: 45),
              ...
              series: <LineSeries<TimePoint, String>>[
                LineSeries<TimePoint, String>(
                    name: 'Ziel ${snapshot.data![0].last.value.toStringAsFixed(0)}°C',
                    // animationDuration: 0,
                    color: (ref.watch(mqttMessagesFamProvider('incubator/on_state')) == 'ON')
                ...
                LineSeries<TimePoint, String>(
                    name: 'Aktuell ${(snapshot.data![1].last.value).toStringAsFixed(0)}°C',
                    // animationDuration: 0,
                    color: Colors.blue,
                    width: 3,
                    enableTooltip: true,
